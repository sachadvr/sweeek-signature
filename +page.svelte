...
import { onMount, beforeUpdate, afterUpdate } from "svelte";
  ...
  onMount(() => {
    json = await fetch('http://localhost:3000').then((res) => res.json).then(() => {
      items = json['issues'].map((e) => {
    if (e['fields']['assignee'] != null) {
      return e['fields']['assignee']['displayName'];
    }
  });
    Auteurs = json = json['issues'].map((e) => {
    if (e['fields']['assignee'] != null) {
      return e['fields']['assignee']['displayName'];
    }
    });
  ...
  export const jsonTest = json['issues'];
  ...
  <div style='font-family: "Source Code Pro","monaco","PT Mono","Courier",monospace; font-size: 40px;'>
    ...
    <div class="w-full h-[100vh] bg-white relative flex items-center justify-center" id="scrollToRoulette">
      <button on:click={() => setWord('Daily - Dashboard').then(setTimeout(()=>{
       ...
       <!-- <div class="z-[19923993] w-full grid grid-cols-6 gap-4 p-4" style="font-family:Quicksand,sans-serif;">
        <div class="absolute bottom-0 left-0 w-full grid grid-cols-4">
        {#each Array.from(Auteurs) as aut}
       <button><div on:click={() => {auteur = aut}}
        style="background: {generateRandomColor()};" class="text-center text-white p-4">{aut}</div></button>
       {/each}
      </div>

        {#each jsonTest as key,value}

        {#if jsonTest[value]['author'] == auteur}
          <div class="flex flex-col p-4 rounded-xl overflow-hidden h-fit text-white" style="box-shadow:  5px 5px 10px #7b7b7b,
          -5px -5px 10px #ffffff; "><span class="bg-blue-400 text-center -mt-[16px] text-white -mx-[16px]">{jsonTest[value]['key'] ?? 'Pas de Nombre'}</span> <span>{jsonTest[value]['summary'] ?? 'Pas de description'}</span> </div>
        {/if}
          {/each} -->
